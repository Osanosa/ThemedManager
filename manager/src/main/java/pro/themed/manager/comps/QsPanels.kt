...
fun QSTileCard() {
    ...
    qsShell.addOnStderrLineListener(
    ...
    fun MyIconButton(overlayname: String, contentdescription: String, iconname: Int) {
        IconButton(
            onClick = {
                qsShell.run(
                    "sed -i 's/@drawable\\/[^\"]*/@drawable\\/$overlayname/g' \"res/drawable/ic_qs_circle.xml\""
                )
            },
            modifier = Modifier.size(testdp.dp).background(color = background),
        ) {
            Image(
    ...
    Column(modifier = Modifier) {
        Row(
            ...
            IconButton(onClick = { qsShell.run("cmd overlay disable themed.qspanel.generic") }) {
                Image(painter = painterResource(R.drawable.reset), contentDescription = null)
            }
        }
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            LaunchedEffect(Unit) {
                style =
                    qsShell
                        .run(
                            """awk -F'bg_' '/<item android:drawable="@drawable\/bg_/ {print $2}' $qsPath/res/drawable/themed_qspanel.xml | sed 's/\" \/>//g'"""
                        )
                        .stdout()
            }
            LaunchedEffect(style) {
                qsShell
                    .run(
                        """sed -i 's/@drawable\/[^"]*/@drawable\/bg_$style/g' "$qsPath/res/drawable/themed_qspanel.xml""""
                    )
                    .log()
            }

            FilterChip(
                colors = FilterChipDefaults.filterChipColors(selectedContainerColor = Yellow),
                shape = CircleShape,
        ...
        LaunchedEffect(Unit) {
            qs_label_container_margin =
                qsShell
                    .run(
                        """awk -F'[<>]' '/<dimen name="qs_label_container_margin">/ {print $3}' $qsPath/res/values/dimens.xml | sed 's/dip//g'"""
                    )
                    .stdout()
            qs_tile_start_padding =
                qsShell
                    .run(
                        """awk -F'[<>]' '/<dimen name="qs_tile_start_padding">/ {print $3}' $qsPath/res/values/dimens.xml | sed 's/dip//g'"""
                    )
                    .stdout()
            columnsPortrait =
                qsShell
                    .run(
                        """awk -F'[<>]' '/<integer name="quick_settings_num_columns">/ {print $3}' $qsPath/res/values/integers.xml"""
                    )
                    .stdout()
            rowsPortrait =
                qsShell
                    .run(
                        """awk -F'[<>]' '/<integer name="quick_settings_max_rows">/ {print $3}' $qsPath/res/values/integers.xml"""
                    )
                    .stdout()
            columnsLandscape =
                qsShell
                    .run(
                        """awk -F'[<>]' '/<integer name="config_qs_columns_landscape">/ {print $3}' $qsPath/res/values/integers.xml"""
                    )
                    .stdout()
            rowsLandscape =
                qsShell
                    .run(
                        """awk -F'[<>]' '/<integer name="config_qs_rows_landscape">/ {print $3}' $qsPath/res/values/integers.xml"""
                    )
                    .stdout()
            qsQuickTileSize =
                qsShell
                    .run(
                        """awk -F'[<>]' '/<dimen name="qs_quick_tile_size">/ {print $3}' $qsPath/res/values/dimens.xml | sed 's/dip//g'"""
                    )
                    .stdout()
            qsTileHeight =
                qsShell
                    .run(
                        """awk -F'[<>]' '/<dimen name="qs_tile_height">/ {print $3}' $qsPath/res/values/dimens.xml | sed 's/dip//g'"""
                    )
                    .stdout()
        }
        Row {
            OutlinedTextField(
                ...
                onValueChange = {
                    ...
                    qsShell.run(
                        """sed -i 's/<dimen name="qs_label_container_margin">[^"]*/<dimen name="qs_label_container_margin">${it}dip/g' $qsPath/res/values/dimens.xml"""
                    )
                },
                keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number),
                label = { Text(text = "qs_label_container_margin", Modifier.basicMarquee()) },
            )
            ...
            OutlinedTextField(
                ...
                onValueChange = {
                    ...
                    qsShell.run(
                        """sed -i 's/<dimen name="qs_tile_start_padding">[^"]*/<dimen name="qs_tile_start_padding">${it}dip/g' $qsPath/res/values/dimens.xml"""
                    )
                },
                keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number),
