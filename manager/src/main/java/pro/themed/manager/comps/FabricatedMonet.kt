...
@Composable
fun FabricatedMonet(scroll: ScrollState) {
    ...
    colorsShell.addOnStderrLineListener(
        object : OnLineListener {
            override fun onLine(line: String) {
                CoroutineScope(Dispatchers.Main).launch { line.log("FM ERROR") }
            }
        }
    )

    colorsShell.addOnCommandResultListener(
        object : OnCommandResultListener {
            override fun onResult(result: Command.Result) {
                CoroutineScope(Dispatchers.Main).launch { result.log("FM RESULT") }
            }
        }
    )
    ...
    var colorsXmlContent by remember {
        mutableStateOf(
            colorsShell
                .run(
                    "cat /data/adb/modules/ThemedProject/onDemandCompiler/fakeMonet/res/values$isDark/colors.xml"
                )
                .stdout
        )
    }
    ...
    LaunchedEffect(selectedColorReference) {
        val colorValue =
            colorsXmlContent
                .find { it.contains("<color name=\"$selectedColorReference\">") }
    ...
    fun getColorValue(colorName: String): Int {
        ...
        return if (
            ...
            colorLine?.substringAfter("#")?.substringBefore("</color>")?.let {
    ...
    fun updateColor(colorName: String, colorValue: Int?) {
        colorsShell.run("cd $colorsPath")
        colorsShell.run(
            """sed -i -r '/$colorName">/ s/>#[0-9A-Fa-f]{6,8}</>#${
    ...
    @Composable
    fun ThemedColor(colorName: String) =
        when {
        ...
        Column(Modifier.animateContentSize()) {
            ...
            Box {
                androidx.compose.animation.AnimatedVisibility(
                    ...
                    Text(text = stringResource(string.monet_tutorial))
                }
                ...
                androidx.compose.animation.AnimatedVisibility(
                    ...
                    Column(
                        ...
                        fun M3Tile(
                            ...
                            var showHexDialog by rememberSaveable { mutableStateOf(false) }
                            var hexInt by remember { mutableStateOf("#" + "%08x".format(color)) }
                            AnimatedVisibility(visible = showHexDialog) {
                                Dialog(onDismissRequest = { showHexDialog = false }) {
                                    CookieCard() {
                                        Column(modifier = Modifier.fillMaxWidth().padding(8.dp)) {
                                            ...
                                            Text(
                                                text =
                                                    if (
                                                        runCatching { Color(hexInt.toColorInt()) }
                                                            .isSuccess
                                                    ) {
                                                        "SAMPLE"
                                                    } else {
                                            ...
                                            OutlinedTextField(
                                                ...
                                                value = hexInt,
                                                singleLine = true,
                                                onValueChange = { hexInt = it },
                                                label = {
                                                    Text(text = colorName, Modifier.basicMarquee())
                                                },
                                                placeholder = {
                                                    Text(
                                                        "#" + "%08x".format(color),
                                            ...
                                            Text(
                                                text =
                                                    "Please note that technically you can enter semi-transparent color, but some apps, for example chrome may stop working, so just leave alpha set to ff"
                                            )
                                            Row(horizontalArrangement = Arrangement.SpaceAround, modifier = Modifier.fillMaxWidth()) {
                                                val context = LocalContext.current
                                                val filePath =
                                                    "/data/adb/modules/ThemedProject/onDemandCompiler/fakeMonet/res/values$isDark/colors.xml"
                                                OutlinedButton(
                                                    onClick = {
                                                        colorsShell.run("am start -a android.intent.action.VIEW -d file://$filePath -t text/plain")
                                                    }
                                                ) {
                                        ...
                                        modifier =
                                            Modifier.width(tilesize)
                                                ...
                                                .combinedClickable(
                                                    onClick = {
                                                        val hex =
                                                            "%08x".format(themedColor.toArgb())
                                                        colorsShell.run("cd $colorsPath")
                                                        colorsShell.run(
                                                            """sed -i -r '/$colorName">/ s/>#[0-9A-Fa-f]{6,8}</>#$hex</g' res/values$isDark/colors.xml"""
                                                        )
                                                        resetColorsXmlContent()
                                                    },
                                                    onLongClick = { showHexDialog = true },
                                                ),
                                        color = it,
                                    ) {
                            ...
                            if (configuration.orientation == Configuration.ORIENTATION_PORTRAIT) {
                                ...
                                Column(
                                    ...
                                    listOf(
                                        ...
                                        .forEach { colorName -> Row { tiles(colorName + "_") } }
                                }
                            }
                        }
                                            ...
                                            Button(
                                                ...
                                                colors =
                                                    ButtonDefaults.buttonColors(
                                                        containerColor =
                                                            Color(getColorValue(it.key + "_500"))
                                                    ),
                                                contentPadding = PaddingValues(0.dp),
                                                onClick = { updateBatchColors(it.key) },
                                            ) {
                                                Text(text = it.value)
                                            }
                                        }
                                    ...
                                    Slider(
                                        ...
                                        modifier =
                                            Modifier.height(16.dp)
                                                ...
                                                .padding(0.dp),
                                        value = hue,
                                        onValueChange = { hue = it.roundToInt().toFloat() },
                                        valueRange = 0f..360f,
                                    ...
                                    Slider(
                                        ...
                                        modifier =
                                            Modifier.background(
                                                ...
                                                .padding(0.dp),
                                        value = saturation,
                                        onValueChange = { saturation = it.roundToInt().toFloat() },
                                        valueRange = 0f..100f,
                                        onValueChangeFinished = {},
                                        steps = 19,
                                    )
                                    ...
                                    Slider(
                                        ...
                                        modifier =
                                            Modifier.background(
                                                ...
                                                .padding(0.dp),
                                        value = lightness,
                                        onValueChange = { lightness = it.roundToInt().toFloat() },
                                        valueRange = -10f..10f,
                                        onValueChangeFinished = {},
                                        steps = 19,
                                    )
                        ...
                        AnimatedVisibility(visible = isColorReferenceDropdownExpanded) {
                            ...
                            fun ReferenceItem(
                                ...
                                Row(
                                    modifier =
                                        Modifier.fillMaxWidth()
                                            .padding(
                                                horizontal = 8.dp,
                                                vertical =
                                                    if (colorName == selectedColorReference) 8.dp
                                                    else 0.dp,
                                            )
                                    ...
                                    Text(
                                        ...
                                        color =
                                            if (colorValue.toColor().luminance() > 0.5f) {
                                                Black
                                            } else {
                                                White
                                            },
                                        modifier =
                                            Modifier.wrapContentWidth(End)
                                                .clip(RoundedCornerShape(8.dp))
                            ...
                            val colorReferences =
                                colorsXmlContent
                                    ...
                                    .map {
                                        val colorName =
                                            it.substringAfter("<color name=\"")
                                                .substringBefore("\">@color/")

                                        val colorReference =
                                            it.substringAfter("@color/")
                                                .substringBefore("</color>")
                            ...
                            Column {
                                ...
                                Row {
                                    listOf("neutral1", "neutral2", "accent1", "accent2", "accent3")
                                        .forEach { accent ->
                                            val containerColor =
                                                Color(
                                                    getColorValue(
                                            ...
                                            Button(
                                                ...
                                                colors =
                                                    ButtonDefaults.buttonColors(
                                                        ...
                                                        contentColor =
                                                            if (containerColor.luminance() > 0.4f) {
                                                                Black
                                                            } else {
                                                                White
                                                            },
                                                ...
                                                onClick = {
                                                    if (selectedColorReference.isBlank()) {
                                                        ...
                                                        selectedMonetColor =
                                                            selectedMonetColor.replaceBeforeLast(
                                                                "_",
                                                                "system_$accent",
                                                            )
                                                        resetColorsXmlContent()
                                                ...
                                                Text(
                                                    text =
                                                        accent.first().uppercase() + accent.last(),
                                                    color =
                                                        if (containerColor.luminance() > 0.4f) {
                                                            Black
                                                        } else {
                                ...
                                FlowRow(
                                    ...
                                    nums.forEach { num ->
                                        val containerColor =
                                            Color(
                                                getColorValue(
                                                    selectedMonetColor.substringBeforeLast("_") +
                                                        "_$num"
                                                )
                                            )
                                        Button(
                        ...
                        Row(Modifier.padding(8.dp)) {
                            OutlinedButton(
                                border = BorderStroke(2.dp, contentcol.copy(0.5f)),
                                onClick = {
                                    Shell.SH.run(
                                        "su -c cmd overlay disable themed.fakemonet.generic"
                                    )
                                    Shell.SH.run(
                                        "su -c cmd overlay enable com.android.systemui:accent"
                                    )
                                    Shell.SH.run(
                                        "su -c cmd overlay enable com.android.systemui:neutral"
                                    )
                                    Shell.SH.run(
                                        "su -c cmd overlay enable com.android.systemui:dynamic"
                                    )
                            ...
                            Button(
                                ...
                                onClick = {
                                    ...
                                    Shell.SH.run(
                                        "su -c cmd overlay disable com.android.systemui:accent"
                                    )
                                    Shell.SH.run(
                                        "su -c cmd overlay disable com.android.systemui:neutral"
                                    )
                                    Shell.SH.run(
                                        "su -c cmd overlay disable com.android.systemui:dynamic"
                                    )
                                    overlayEnable("misc.flagmonet")

                                    colorsShell.run(
                                        """cmd overlay enable themed.fakemonet.generic"""
                                    )
                                    ...
                                    Icon(
                                        ...
                                        imageVector =
                                            ImageVector.vectorResource(
                                                id = drawable.arrow_right_alt_48px
                                            ),
                                        contentDescription = "",
                                    )
                                }
