    ...
    Card(
        ...
        Card(
            ...
            Column(modifier = Modifier.animateContentSize()) {
                val shell = Shell("su")
                shell.addOnCommandResultListener(
                ...
                Box {
                    androidx.compose.animation.AnimatedVisibility(
                        ...
                        Text(
                            text =
                                "This is the accent picker, " +
                                    "it is linked via overlay to framework-res, " +
                                    "overlaying some of the colors, " +
                                    "mainly those that were before A12. " +
                                    "This won't change background colors, " +
                                    "you can use FakeMonet for that."
                        )
                    }
                        ...
                        Column(modifier = Modifier.padding(horizontal = 8.dp)) {
                            ...
                            Row(Modifier.padding(vertical = 4.dp)) {
                                OutlinedButton(
                                    ...
                                    onClick = {
                                        shell.run(
                                            "cmd overlay disable themed.accent.generic ; pm uninstall themed.accent.generic"
                                        )
                                        hue = 45f
                                        saturation = 100f
                                        lightness = 50f
                                ...
                                Button(
                                    ...
                                    onClick = {
                                        shell.run(
                                            "cd /data/adb/modules/ThemedProject/onDemandCompiler/staticAccent"
                                        )
                                        shell.run(
                                            """sed -i 's/>#\([0-9A-Fa-f]\{8\}\)</>#$hex</g' "res/values$isDark/colors.xml""""
                                        )
                                        buildOverlay(
                                            "/data/adb/modules/ThemedProject/onDemandCompiler/staticAccent"
                                        )
                                    ...
                                    Row(verticalAlignment = Alignment.CenterVertically) {
                                        Text(
                                            text = "Build, ${hex.removePrefix("ff")}",
                                            color =
                                                if (lightness > 50f) {
                                                    Color.Black
                                                } else {
