...
class PrivilegedCommandHelper {
     ...
     */
    suspend fun checkAvailability(context: Context, skipRootIfDenied: Boolean = false): AvailabilityStatus = withContext(Dispatchers.IO) {
        // Check root availability - first check if su binary exists without triggering permission dialog
        val (rootAvailable, rootPermissionDenied, rootVersion) = if (skipRootIfDenied && lastRootPermissionDenied) {
            // Skip root check if previously denied and this is a periodic check
            Log.d("PrivilegedHelper", "Skipping root check due to previous denial")
            Triple(false, true, null)
        } else {
            try {
                Log.d("PrivilegedHelper", "Checking if su binary is available...")
                // First check if su binary exists by checking version (doesn't require root permissions)
                val versionResult = Shell.SH.run("su -v")
                Log.d("PrivilegedHelper", "Su version check: exitCode=${versionResult.exitCode}, stdout='${versionResult.stdout()}'")
                
                if (versionResult.exitCode == 0 && versionResult.stdout().trim().isNotEmpty()) {
                    val version = versionResult.stdout().trim()
                    Log.d("PrivilegedHelper", "Su binary available with version: $version")
                    // Su binary exists, but we don't know about permissions yet
                    // Mark as available but permission status unknown (will be checked when needed)
                    Triple(true, false, version)
                } else {
                    Log.d("PrivilegedHelper", "Su binary not available")
                    Triple(false, false, null)
                }
            } catch (e: Exception) {
                Log.d("PrivilegedHelper", "Su version check exception: ${e.message}")
                Triple(false, false, null)
            }
        }
        ...
        val shizukuAvailable = if (shizukuInstalled) {
            try {
                Shizuku.pingBinder()
            } catch (e: Exception) {
                false
        ...
        Log.d("PrivilegedHelper", "Final status: $status")
        status
    }
     ...
     */
    suspend fun testRootPermissions(): Boolean = withContext(Dispatchers.IO) {
        try {
            Log.d("PrivilegedHelper", "Testing root permissions...")
            val testResult = Shell.SH.run("su -c whoami")
            Log.d("PrivilegedHelper", "Root permission test: exitCode=${testResult.exitCode}, stdout='${testResult.stdout()}', stderr='${testResult.stderr()}'")
            ...
            if (!success) {
                ...
                lastRootPermissionDenied = stderr.contains("permission denied") ||
            ...
            Log.d("PrivilegedHelper", "Root permission test exception: ${e.message}")
            // Check if exception indicates permission denial
            val exceptionMessage = e.message?.lowercase() ?: ""
            lastRootPermissionDenied = exceptionMessage.contains("permission denied") ||
