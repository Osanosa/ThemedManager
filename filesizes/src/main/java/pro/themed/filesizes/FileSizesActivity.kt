...
class FileSizesActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContent {
            lsShell.addOnStderrLineListener(
            ...
            val listOfStorageVolume by remember {
                mutableStateOf(
                    context.externalCacheDirs.map {
                        it?.path?.removeSuffix("/Android/data/pro.themed.filesizes/cache")
                            ?: "/sdcard"
                    }
                )
            }
            ...
            lsShell.run(command) {
                onStdErr = { line: String ->
                    ...
                    command =
                        when {
                            line.contains("Unknown option '-1'") ->
                                command.replace("-1", "") // -1 lists one file per line
                            line.contains("Unknown option '-p'") ->
                                command.replace("-p", "") // -p adds a / to the end of directories
                            line.contains("Unknown option '-L'") ->
                                command.replace(
                                    "-L",
                                    ""
                                ) // -L follows symbolic links to their targets
                            else -> command
                        }
                }
            }

            suspend fun du(path: String) =
            ...
            suspend fun ls(): Unit = coroutineScope {
                ...
                lsShell.run(command) {
                    onStdOut = { line: String ->
                        ...
                        // if the file or directory is a symbolic link, the name is the target of
                        // the symbolic link
                        ...
                        if (command.contains("-p")) {
                            if (currentList[fixedName]?.du != true)
                        ...
                        currentList.keys
                            .filter { it.startsWith(fixedName) }
                            .forEach { it.log("ITEM") }
                    }
                    onStdErr = { line: String ->
                        // if there is an error, log it
            ...
            lsShell.addOnCommandResultListener(
                        ...
                        LazyColumn(
                            ...
                            items(
                                ...
                                Box(Modifier.animateItem()) {
                                    ...
                                    Item(
                                        item.first.removePrefix(currentPath).removePrefix("/"),
                                        item.second,
                                        mimeType,
                                        onClick = {
                                        ...
                                        fullPath = item.first,
                                        onDeletion = { currentList.remove(item.first) }
                                    )
                                }
                            }
                        }
